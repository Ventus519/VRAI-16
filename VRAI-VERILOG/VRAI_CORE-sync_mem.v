...
module VRAI_CORE (
    ...
    output reg MAIN_STRH,
    output reg MAIN_STRW,
    output reg MAIN_LDH,
    output reg MAIN_LDW,

    output reg DEVICES_STRH,
    output reg DEVICES_STRW,
    output reg DEVICES_LDH,
    output reg DEVICES_LDW,

    output reg INVALID_MEM,
...
wire MEM_INVALID;
assign STRH_MAIN = MEM_MAIN_STRH & UNIT_MEM_EN;
assign STRW_MAIN = MEM_MAIN_STRW & UNIT_MEM_EN;
assign LDH_MAIN = MEM_MAIN_LDH & UNIT_MEM_EN;
assign LDW_MAIN = MEM_MAIN_LDW & UNIT_MEM_EN;

assign STRH_DEVICES = MEM_DEVICES_STRH & UNIT_MEM_EN;
assign STRW_DEVICES = MEM_DEVICES_STRW & UNIT_MEM_EN;
assign LDH_DEVICES = MEM_DEVICES_LDH & UNIT_MEM_EN;
assign LDW_DEVICES = MEM_DEVICES_LDW & UNIT_MEM_EN;

assign MEM_INVALID = MEM_INVALID_0 & UNIT_MEM_EN;


wire OUT_MEM_DATA;

//technically not needed since this condition is checked when
//streaming to mem_data_out and mem_addr_out. Keeping it for clarity.
...
COUNTER counter (
    ...
    .ENABLE(~HALT_EXECUTION & ~STALL_EXECUTION),
...
wire [3:0] SW_ALU_DEST_0;
wire [3:0] SW_SYS_DEST_0; //doesnt do anything yet
wire [3:0] SW_BIT_DEST_0;
wire [3:0] SW_MEM_DEST_0;
wire [3:0] DEST;
...
wire [15:0] SW_SYS_RESULT_0;

assign SW_SYS_RESULT_0 = SYS_COUNT_RESULT; //expands into an or tree as more sys results are implemented

wire [15:0] SW_ALU_RESULT_0;
wire [15:0] SW_BIT_RESULT_0;
wire [15:0] SW_MEM_RESULT_0;
...
reg [15:0] SW_SYS_RESULT;
reg [15:0] SW_ALU_RESULT;
reg [15:0] SW_BIT_RESULT;
reg [15:0] SW_MEM_RESULT;
...
assign DEST_WRITE = (UNIT_ALU_EN | UNIT_BIT_EN | MEM_WRITE_DEST | SYS_WRITE_DEST) & (DEST != 4'h0);
reg WRITE_DEST;

integer i;


localparam STATE_INSTR_FETCH_REQ = 0,
           STATE_INSTR_FETCH_WAIT = 1,
           STATE_INSTR_FETCH_COMPLETE = 2,
           STATE_EXECUTE = 3,
           STATE_MEM_WAIT = 5,
           STATE_STREAM = 4;
...
always @(posedge CLK or posedge RESET) begin
    if (RESET) begin
        ...
        HALT_EXECUTION <= 0;
        STATE <= STATE_INSTR_FETCH_REQ;
    end else begin
        //The main area for streaming the actual interesting parts 
        
        case (STATE)
            STATE_INSTR_FETCH_REQ: begin
                STALL_EXECUTION <= 1; //stalls the counter until the instruction is completed
                STATE <= STATE_INSTR_FETCH_WAIT;
            end
            STATE_INSTR_FETCH_WAIT: begin
                //sad
                STATE <= STATE_INSTR_FETCH_COMPLETE;
            end
            STATE_INSTR_FETCH_COMPLETE: begin
                INSTR <= INSTR_STREAM;
                STATE <= STATE_EXECUTE;
            end

            STATE_EXECUTE: begin
                SW_ALU_DEST <= SW_ALU_DEST_0;
                SW_SYS_DEST <= SW_SYS_DEST_0;
                SW_BIT_DEST <= SW_BIT_DEST_0;
                SW_MEM_DEST <= SW_MEM_DEST_0;

                SW_ALU_RESULT <= SW_ALU_RESULT_0;
                SW_BIT_RESULT <= SW_BIT_RESULT_0;
                SW_SYS_RESULT <= SW_SYS_RESULT_0;

                MEM_EN <= UNIT_MEM_EN;

                WRITE_DEST <= DEST_WRITE;
                ...
                if (UNIT_MEM_EN) begin
                    ...
                    DEVICES_STRH <= STRH_DEVICES;
                    DEVICES_STRW <= STRW_DEVICES;
                    DEVICES_LDH <= LDH_DEVICES;
                    DEVICES_LDW <= LDW_DEVICES;

                    INVALID_MEM <= MEM_INVALID;

                    MEM_DATA_OUT <= OUT_MEM_DATA;
                    MEM_ADDR_OUT <= OUT_MEM_ADDR;
                end
